...
| 기록 규모 | 잔차 29세션 · 채점 28일(적중률 39.3%, 평균 −3.25bp, t=−0.31) · freeze 29(live 1 · late 3 · reconstructed 25) |
| 2단계 개시 | **60세션 = 2026-10-21 세션, KST 2026-10-22 07:00.** 단 그 시점은 예비 추정치까지다 — 7장 2단계 머리말의 검정력 표 참조 |
...
**먼저: 60거래일은 "코드를 돌려보는 시점"이지 "결론이 나오는 시점"이 아니다.**
2026-09-09에 지금 데이터로 검정력을 실측했다. 60세션 도달은 **2026-10-21(수)
세션, 한국시간 2026-10-22 07:00 리포트**다(실측 지연 감안 06:40~07:00).

| 방식 | 일별 추정치 | 표준편차 | 현재 t (n=28) | `|t|>3` 필요 표본 |
|---|---|---|---|---|
| 분위 포트폴리오 스프레드 (4번 블록) | −3.25bp | 54.69bp | −0.31 | ~99년 |
| **Fama-MacBeth 계수** (표준화 sent_w 1SD당) | −1.40bp | 15.70bp | −0.47 | ~1,128일(4.5년) |

**분위 스프레드와 FM 회귀의 효율이 20배 넘게 차이 난다.** 스프레드는 상하위
59종목만 쓰고 FM은 세션당 355종목을 전부 쓰기 때문이다. 11번을 먼저 하라는
순서가 검정력 면에서도 맞다.

T별로 `|t|>3` 을 넘기려면 계수가 얼마나 커야 하는가(FM 기준):

| T | 필요 계수 | 현재 점추정으로 얻는 `|t|` |
|---|---|---|
| **60** | **6.08bp / 1SD** | 0.69 |
| 125 | 4.21bp | 1.00 |
| 250 | 2.98bp | 1.41 |
...
뉴스 감성 1SD당 익일 잔차 6bp는 나오면 아주 좋은 축이고 `[검증 필요]`,
현재 점추정은 −1.40bp로 부호도 반대다(n=28이라 점추정 자체가 노이즈다).

**그래서 60세션에 할 수 있는 것과 없는 것을 나눠 둔다.**
- 할 수 있다: **12번 event study.** 관측 단위가 '일'이 아니라 '사건'이라 60일이면
  수만 건이다. 단 매크로 이벤트는 event-date clustering 때문에 유효 N이 결국
  일수로 되돌아가므로 **기업 고유 이벤트(실적·M&A)부터** 본다.
  그리고 코드 가동 확인, 요일·토픽별 기술통계
- 할 수 없다: 11번의 유의성 결론. 13번 block bootstrap(20일 블록이면 60일은
  **3블록**이다)
...
16. **세션 누락 감지·복구.** 설계 완료, 구현 없음 → `docs/SESSION_GAPS.md`.
    ...
    **스케줄 이벤트 누락 관측 기록.** GitHub은 스케줄 발사를 보장하지 않는다.
    슬롯을 늘릴지 판단하려면 사례를 세야 하므로 여기에 누적한다.

    | 날짜 | 누락 슬롯 | 결과 |
    |---|---|---|
    | 2026-08-06 | **두 슬롯 동시** | 세션 손실. 다음날 사람이 눈으로 발견 |
    | 2026-09-09 | 20:50 UTC 한 슬롯 | **22:20 슬롯이 받아 정상 처리.** 무해 |

    **슬롯을 3개로 늘리는 건 공짜가 아니다.** 실행당 AV 호출이 배치 7 + 이어받기
    최대 5 = 12회다. 2슬롯이면 최악 24회로 무료 한도 25 안이지만 **3슬롯이면 36회로
    넘는다.** freeze 가드 덕에 첫 실행이 성공하면 뒤 슬롯은 no-op이라 실제로는
    거의 안 쓰이지만, 가격 미확정(종료코드 2)으로 freeze 전에 빠지면 다시 수집한다.
    즉 **한도를 넘기는 경우가 정확히 슬롯이 필요한 경우**다.
    그러므로 증설 기준은 '한 슬롯 누락'이 아니라 **'두 슬롯 동시 누락'의 재발**로
    잡는다. 현재 1건. 재발하면 슬롯 증설보다 계층 1(갭 자동 메움)이 먼저다 --
    그쪽은 한도를 하루에 몰아 쓰지 않는다.
...
17. **세션 freeze — OOS 원본 보존. 미구현. 지금 가장 급한 항목이다.**
    ...
    **잔차 재계산 (2026-09-09). `--residuals` 플래그로 같은 스크립트가 한다.**
    월요일 수익률 버그를 고치고 가격을 다시 받았지만 **그 전에 계산된 잔차는
    월요일이 빠진 베타 추정창 위에 서 있었다.** 표본 전반부와 후반부가 다른
    방식으로 만들어지는 셈이라 2단계 분석 전에 정리했다.

    결과: 29세션 중 **24세션의 값이 바뀌었다.** 안 바뀐 5개는 정확히 가격 수정
    이후에 계산된 세션들이다(08-03·08-10·08-17·08-24 복구분 + 09-08 정기 실행).
    |차이| 중위의 중위 15.01bp, 최저 상관 0.8333.
    채점은 spread 변화 중위 3.16bp, hit 반전 1건(08-28)으로 사전 교체 때(7건)보다
    훨씬 작다.

    **변화가 가장 큰 3개는 월요일 버그 때문이 아니다.** 07-29·07-30·07-31은
    지금 팩터 source가 `french`(확정치)인 유일한 구간이고, 그때는 `etf_proxy`로
    계산됐다. 즉 8장 2번(Ken French 소급 갱신)의 효과다. 재계산은 두 종류의
    ex-post 수정을 함께 반영한다 -- 둘 다 latest 층에서는 정상이다.
    세션별 상세는 `data/rescore.meta.json` 의 `residuals_rebuilt.per_session`.

    순서를 이 스크립트가 소유한다: **residuals → signals → scorecard →
    scorecard.json.** 두 군데서 따로 만들면 어긋난다.

    마지막 항목은 재계산하다 발견한 것이다. `data/scorecard.json` 은 대시보드용
    **파생 뷰**인데 `append_scorecard_json` 이 덧붙이기만 해서 채점을 다시
    계산해도 옛 값이 남는다. 실측: 23항목이 남아 있었고 07-30이 json −97.27 대
    parquet −50.05 였다 -- **같은 레포 안에 서로 다른 채점표가 두 개 있었다.**
    이제 재계산 끝에 원본에서 다시 만든다(그때 git 해시 `rev` 는 살려 옮긴다).

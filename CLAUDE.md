...
16. **세션 누락 감지·복구.** 설계 완료, 구현 없음 → `docs/SESSION_GAPS.md`.
    ...
    **남은 것: 네 세션의 실제 복구 판단.** 가격·팩터는 고쳐졌으므로 이제
    `--session` 재실행이 잔차·채점을 만들 수 있다. 다만 뉴스가 문제다 —
    4~5주 지난 날짜라 AV가 그 구간을 돌려줄지 불확실하고, 뉴스가 비면 신호가
    비어 그 세션은 '가격은 있고 신호는 없는' 반쪽 기록이 된다. 그리고
    `provenance` 는 `late` 로 찍힌다(실시간이 아니므로 정상이다).
    복구할지, 갭으로 남길지는 그 반쪽 기록이 채점 통계에 들어가도 되는가의
    문제다. **판단 전에 `python scripts/diagnose_news.py` 로 AV의 과거 구간
    응답을 먼저 볼 것.**

...
BANNED = ["혼조세", "관망세", "눈치보기", "훈풍", "온기", "숨고르기", "기대감이 커지",
          ...
          # 아래는 openai 전환 첫 원고(2026-07-30)에서 실제로 나온 표현들이다.
          # 정보량이 없거나 약한 인과를 몰래 주장한다. 특히 '시사한다'는 상관을
          # 원인처럼 읽히게 만든다 -- 이 프로젝트가 피하려는 정확히 그 지점이다.
          "시사한다", "조정을 받", "두드러진 성과", "견인했", "영향으로",
          "이끌었다", "호재로", "악재로"]

# '때문에'는 일부러 넣지 않았다. 시장 해설에서는 인과 주장이지만
# "다중검정 때문에 2.0 기준으로는 우연히 유의한 결과가 대량 생산된다"처럼
# **통계 방법론을 설명하는 정당한 용법**이 있다(weekly_builder에 실제로 있고,
# 넣었더니 test_weekly_analytics가 잡아냈다). 기계적 검사는 거의 항상 틀린
# 표현만 담고, 문맥 판단이 필요한 것은 LLM 프롬프트 쪽에서 금지한다.
...
def build_markdown(ctx: dict, narrative: str, chart_files: list[str], cfg) -> str:
    ...
    # 용어 표는 뺐다. 방법론 공지글이 생겼으므로 매일 같은 표를 반복할 이유가 없다
    # (유사문서 위험도 줄어든다). 잔차·z·모멘텀 설명은 각 블록 안에 남아 있어
    # 글 자체는 여전히 자기완결적이다.
    mu = str(cfg.get_path("report.methodology_url", "") or "").strip()
    if mu:
        A(SECTION)
        A(f"_팩터·잔차·z 같은 용어와 이 기록의 한계를 처음부터 정리한 안내 글이 있다 → "
          f"**[이 블로그를 읽는 방법]({mu})**_")
        A("")

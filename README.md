...
| **네이버** | 다른 독자층 | 붙여넣기 2분 | **뉴스 중심 리포트** |

네이버는 티스토리의 요약본이 아니다. 요약본은 원문의 열화판이라 읽을 이유가 없었고
유사문서 위험도 있었다. 지금은 **서술 축이 다르다** — 티스토리는 숫자가 주인공이고
네이버는 뉴스가 주인공이다. 같은 데이터를 쓰지만 문장이 겹치지 않는다.

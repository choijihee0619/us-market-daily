...
| **매주 월요일 세션이 통째로 누락** | cron 요일이 `2-6`(화~토 UTC) — 월요일 실행이 없고, 화요일 실행은 화요일장을 잡는다 | **무증상** |
| 제목이 빈 리포트가 생성됨 | 거래소는 마감했는데 yfinance가 Close를 아직 NaN으로 준다. 게이트가 달력만 봤다 | 제목 공백 |
...
- **성공 로그에는 '돌지 않은 날'이 남지 않는다.** 월요일 누락은 워크플로가 5회 연속
  `success`인 상태에서 발생했다. 실패를 감시하는 것과 **누락을 감시하는 것은 다른
  일이다.** 전자는 로그를, 후자는 산출물의 연속성을 봐야 한다.
...
| 스크립트 | 주기 | 용도 |
|---|---|---|
| `run_daily.py` | 매일 | 수집 → 잔차 → 리포트 → 채널 산출물 (현재 GitHub만) |
...
## 3채널 구조 — 현재는 GitHub 단독

> **2026-09-08부터 티스토리·네이버 수동 발행을 중단했다.** 40거래일 동안 실제로
> 발행된 글이 0편이었다(`data/post_urls.json` 없음). `config.yaml` 의
> `report.channels: [github]` 한 줄이고, 두 채널의 코드는 그대로 남아 있다 —
> 목록에 이름을 다시 넣으면 아래 구조가 그대로 복귀한다.
> 아래 설명은 그 복귀 시점의 설계 문서로 남긴다.
...
```
scripts/
├── run_daily.py          매일. 수집 -> 잔차 -> 리포트 -> 채널 산출물
